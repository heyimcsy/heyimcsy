# 🧝🏻‍♀️ 안녕하세요 프론트엔드 개발자 최서윤입니다. 🧝🏻‍♀️

## MY GIT ANIMALS
<div align="center">
  <a href="https://github.com/devxb/gitanimals">
    <img src="https://render.gitanimals.org/farms/{heyimcsy}"/>
  </a>
</div>

## 👋 About Me

안녕하세요.
React, Next.js, TypeScript를 중심으로 개발하는 프론트엔드 개발자 최서윤입니다.

사용자가 자연스럽게 사용할 수 있는 UI를 만드는 것을 좋아하며,
문제를 분석하고 성능을 개선하는 과정에서 큰 재미를 느낍니다.

최근에는 Java, Spring Boot, 백엔드 아키텍처를 공부하며
프론트엔드를 넘어 서비스 전체를 이해하는 개발자가 되는 것을 목표로 하고 있습니다.

---

Frontend Developer focused on React, Next.js and TypeScript.

I enjoy building interactive user interfaces,
solving challenging problems, and continuously learning new technologies.

Currently learning Java, Spring Boot and backend architecture.

## 🚀 Featured Projects

### 🏥 Hospital Reservation

Next.js · TypeScript · Kakao Map · TanStack Query · Shadcn-ui

- 병원 예약 서비스
- 지도 기반 검색
- Drawer UI
- 예약 시스템

---

### 🤖 Robot Map Editor

React · TypeScript · Three.js · Canvas

- 지도 편집기
- Polygon 편집
- Node & Link Editor
- Texture Export

## MY SKILLS
[![My Skills](https://skillicons.dev/icons?i=js,html,css)](https://skillicons.dev)
-------
[![My Skills](https://skillicons.dev/icons?i=react,vue,nextjs)](https://skillicons.dev)
-------
[![My Skills](https://skillicons.dev/icons?i=threejs,ts,jest)](https://skillicons.dev)
-------
[![My Skills](https://skillicons.dev/icons?i=styledcomponents,sass,materialui,tailwind, shadcn)](https://skillicons.dev)
-------
[![My Skills](https://skillicons.dev/icons?i=docker,postman,yarn,npm)](https://skillicons.dev)

## 🌱 Currently Learning

- Java & Spring Boot
- Backend Architecture
- Performance Optimization
- vibe coding

## 📫 Contact

📧 cinnamon1995@naver.com

💼 https://potato-admin.shop/my-portfolio/resume

<!--
**heyimcsy/heyimcsy** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.


Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
